...
public class EncryptionUtil1 {
	...
	public static String authcode(String $string, String $operation, String $key,int $expiry ) {
		...
		if($operation.equals("DECODE")) {
			...
			if ((NumberUtils.toInt(substr($result.toString(), 0, 10)) == 0 || NumberUtils.toLong(substr($result.toString(), 0, 10)) - time() > 0) && substr($result.toString(), 10, 16).equals(substr(md5(substr($result.toString(), 26) + $keyb), 0, 16))) {
