(function($){
	var bigAutocomplete = new function(){
		...
		this.autocomplete = function(param){
			...
			$this.keyup(function(event) {
				...
				if(!isFunctionalKey && (!ctrl || (ctrl && k == 67) || (ctrl && k == 88)) ){
					...
					if(data != null && $.isArray(data) ){
						...
						makeContAndShow(data_, config.url);
					}else if(url != null && url != ""){//ajax请求数据
						$.post(url,{keyword:keyword_},function(result){
							makeContAndShow(result.data, config.url)
						},"json")
					}
			...
			//组装下拉框html内容并显示
			function makeContAndShow(data_, url){
				if(data_ == null || data_.length <=0 ){
					return;
				}
				
				var cont = "<table><tbody>";
				if(url.indexOf('goods-words') > -1){
					for(var i=0;i<data_.length;i++){
						cont += "<tr><td><div>" + data_[i].title + "</div><span>约" + data_[i].goods_num + "个商品</span></td></tr>"
					}
				}else{
					for(var i=0;i<data_.length;i++){
						cont += "<tr><td><div>" + data_[i].store_name + "</div><span></span></td></tr>"
					}
				}
				cont += "</tbody></table>";
