...
var CheckOut={
	...
	bindAddrEvent:function(){
		...
		$("#saveAddrBtn").click(function(){
			...
			$.ajax({
				...
				data: {address_id: addrid, "cartStoreIds": storeIds},
	...
	bindPaymentEvent:function(){
		...
		$("#savePaymentBtn").click(function(){
			 ...
			 /************ 2015/10/25 humaodong ****************/
			 if (name == '货到付款') {
				if ($('.not_send input[name=shipDay]').is(':checked')) {
					$('input[name=shipDay]:eq(0)').prop('checked',true);
				...
				$('.not_send').hide();
			 	...
			 	$('.gome-wrapper').hide();
			 	$('input.useBalance').prop('checked',false).change();
			 	$('select.bonus').val('0-0').change();
			 } else {
				 $('.not_send').show();
			 	$('.gome-wrapper').show();
			 }
	...
	createOrder:function(btn){
		...
		var options = {
			...
			success : function(result) {
	 			...
	 			}else if(result.result==10) //购物车内容发送改变
 				{
	 				$.Loading.hide();
				...
				} else if (result.result == 11) //购物车内容发送改变
				{
					$.Loading.hide();
