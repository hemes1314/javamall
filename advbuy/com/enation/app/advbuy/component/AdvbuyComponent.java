package com.enation.app.advbuy.component;
 ...
 */
@Component
public class AdvbuyComponent implements IComponent {
	...
	/**
	 * 启动组件
	 * 1.执行组件更改数据库（添加预售相关表、修改商品、标签表结构）。
	 * 2.执行sql修改演示数据。
	 * 3.添加预售菜单（促销）。
	...
	@Override
	public void install() {
		DBSolutionFactory.dbImport("file:com/enation/app/advbuy/component/advbuy_install.xml", "es_");
		this.daoSupport.execute("update es_goods set is_advbuy=0");
	       //hp清除缓存
	 ...
	 * 卸载组件
	 * 1.删除后台菜单
	 * 2.因怕影响已有数据已经添加如预售所以这里并没有去删除数据库字段以及表。
	 * 3.修改菜单将商品标签设置url还原。
	 */
	@Override
	...
	private void addMenu(){
		...
		menu.setTitle("预售活动");
		menu.setPid(menu_id);
		menu.setUrl("/shop/admin/advBuyAct!list.do");
		this.menuManager.add(menu);
		/*
		//添加子目录:预售地区
		menu.setTitle("预售地区");
		menu.setUrl("/shop/admin/advBuyArea!list.do");
		this.menuManager.add(menu);
		//添加子目录:预售分类
		menu.setTitle("预售分类");
		menu.setUrl("/shop/admin/advBuyCat!list.do");
		this.menuManager.add(menu);
		*/
		//添加导航栏：预售
		SiteMenu siteMenu = siteMenuManager.get("预售");
		if (siteMenu == null) {
			siteMenu = new SiteMenu();
			siteMenu.setName("预售");
			siteMenu.setParentid(0);
			siteMenu.setUrl("advbuy/advbuy.html");
			siteMenu.setSort(6);
			siteMenu.setTarget("");
	...
	private void delete_menu(){
		...
		//删除主目录：预售管理
		Menu menu =  menuManager.get("预售管理");
		if(menu!=null){
			int addmenuid =menu.getId();
			this.authActionManager.deleteMenu(superAdminAuthId, new Integer[] {addmenuid });
			this.menuManager.delete("预售管理");
		}
		//删除子目录：预售活动
		 menu =  menuManager.get("预售活动");
		if(menu!=null){
			int addmenuid =menu.getId();
			this.authActionManager.deleteMenu(superAdminAuthId, new Integer[] {addmenuid });
			this.menuManager.delete("预售活动");
		}
		//删除子目录：预售地区
		 menu =  menuManager.get("预售地区");
		if(menu!=null){
			int addmenuid =menu.getId();
			this.authActionManager.deleteMenu(superAdminAuthId, new Integer[] {addmenuid });
			this.menuManager.delete("预售地区");
		}
		//删除子目录：预售分类
		 menu =  menuManager.get("预售分类");
		if(menu!=null){
			int addmenuid =menu.getId();
			this.authActionManager.deleteMenu(superAdminAuthId, new Integer[] {addmenuid });
			this.menuManager.delete("预售分类");
		}
		//删除导航栏：预售
		SiteMenu siteMenu= siteMenuManager.get("预售");
	...
	public void editMenu(){
		...
		menu.setUrl("/shop/admin/advBuyTag!list.do");
