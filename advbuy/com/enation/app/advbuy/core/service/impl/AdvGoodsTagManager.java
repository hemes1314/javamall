...
@Component
public class AdvGoodsTagManager implements IAdvGoodsTagManager{
	...
	public Page list(int pageNo, int pageSize) {
		...
		return this.daoSupport.queryForPage("select * from es_tags where store_id is NULL or is_advbuy is not NULL order by tag_id", pageNo, pageSize);
	}
	/**
	 * 获取列表Sql
	 * @param tag_id
	...
	private String getListSql(int tag_id) {
		String 	sql = "select g.*,b.name as brand_name ,t.name as type_name,c.name as cat_name  from es_goods g left join es_goods_cat c on g.cat_id=c.cat_id left join es_brand  b on g.brand_id = b.brand_id and b.disabled=0 left join es_goods_type";
				sql+= " t on g.type_id =t.type_id  where g.goods_type = 'normal' and g.disabled=0 and goods_id in (select goods_id from es_advbuy_goods where act_id=(select act_id from es_advbuy_active where act_tag_id="+tag_id+"))";
		return sql;
	}
	/*
	 * (non-Javadoc)
	 * @see com.enation.app.b2b2c.core.service.goods.IB2b2cGoodsTagManager#advBuyList(int, java.util.Map, int, int, java.lang.String, java.lang.String)
	 */
	@Override
	public Page advBuyList(int tagid, Map goodsMap, int page, int pageSize,
	...
	public List listGoods(String catid, String tagid, String goodsnum) {
		...
		sql.append("select g.*,gb.price as gb_price,gb.original_price as original_price from es_tag_rel r LEFT JOIN es_goods g ON g.goods_id=r.rel_id INNER JOIN es_advbuy_goods gb ON gb.goods_id=g.goods_id where gb.gb_status=1 and g.disabled=0 and g.market_enable=1");
