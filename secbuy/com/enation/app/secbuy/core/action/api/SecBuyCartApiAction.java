package com.enation.app.secbuy.core.action.api;
...
/**
 * 
 * @ClassName: SecBuyCartApiAction 
 * @Description: 秒拍购物车API 
 * @author TALON 
 * @date 2015-7-31 上午12:48:32 
 *
 */
@ParentPackage("shop_default")
@Namespace("/api/secbuy")
@Action("cart")
@Component
public class SecBuyCartApiAction extends WWAction{
	 ...
	 * @param product 货品
	 * 1.判断购买的秒拍货品库存是否充足
	 * 2.判断用户购买的秒拍商品已经超过了商品的限购数量（此步需要去完善应该去库中去查询一下是否已经添加过秒拍商品，应该按库中为准）
	 * 3.将秒拍商品放置购物车中
	 * @return 成功或失败
	 */
	private boolean addProductToCart(Product product){
		...
		if(product!=null){
			try{
				if(secbuy.getGoods_num()<num){
					throw new RuntimeException("抱歉！您所选择的货品库存不足。");
				}
				if(secbuy.getLimit_num()>num){
