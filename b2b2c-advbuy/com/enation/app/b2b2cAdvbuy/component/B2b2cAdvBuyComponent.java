package com.enation.app.b2b2cAdvbuy.component;
 ...
 */
@Component
public class B2b2cAdvBuyComponent implements IComponent{
	...
	public void install() {
		...
		//修改数据库结构
		DBSolutionFactory.dbImport("file:com/enation/app/b2b2cAdvbuy/component/b2b2cAdvbuy_install.xml", "es_");	
		
		//安装示例数据
		String app_apth = StringUtil.getRootPath();
		String dataSqlPath = app_apth+ "/b2b2c/admin/advbuy/example_data.xml";
		...
		try {	
			...
			InstallUtil.putMessaage("正在安装预售基础数据...");
			//开始安装
			...
			//修改演示数据
			daoSupport.execute("update es_goods set is_advbuy=1 where goods_id in (select goods_id from es_advbuy_goods)");
			try {
                //hp清除缓存
                com.enation.framework.cache.ICache  iCache = CacheFactory.getCache(SystemConstants.PRE_GOODS);
                
                String  sql1="SELECT * from es_advbuy_goods ";
	...
	private void editMenu(){
		Menu menu=menuManager.get("预售活动");
		menu.setUrl("/b2b2c/admin/advBuy!list.do");
