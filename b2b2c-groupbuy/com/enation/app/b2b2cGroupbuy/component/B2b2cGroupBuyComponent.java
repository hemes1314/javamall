package com.enation.app.b2b2cGroupbuy.component;
 ...
 */
@Component
public class B2b2cGroupBuyComponent implements IComponent{
	...
	public void install() {
		...
		//修改数据库结构
		DBSolutionFactory.dbImport("file:com/enation/app/b2b2cGroupbuy/component/b2b2cGroupbuy_install.xml", "es_");	
		
		//安装示例数据
		String app_apth = StringUtil.getRootPath();
		String dataSqlPath = app_apth+ "/b2b2c/admin/groupbuy/example_data.xml";
		...
		try {	
			...
			InstallUtil.putMessaage("正在安装团购基础数据...");
			//开始安装
			...
			//修改演示数据
			daoSupport.execute("update es_goods set is_groupbuy=1 where goods_id in (select goods_id from es_groupbuy_goods)");
			try {
                //hp清除缓存
                com.enation.framework.cache.ICache  iCache = CacheFactory.getCache(SystemConstants.PRE_GOODS);
                
                String  sql1="SELECT * from es_groupbuy_goods ";
	...
	private void editMenu(){
		Menu menu=menuManager.get("闪购活动");
		menu.setUrl("/b2b2c/admin/groupBuy!list.do");
